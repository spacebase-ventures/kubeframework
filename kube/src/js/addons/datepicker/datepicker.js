(function ($K) {
  ...
  $K.add("module", "datepicker", {
    ...
    _buildSelectedDate: function () {
      ...
      // set from params
      if (this.value === "") {
        this.selected.year = this.params.year
          ? this.params.year
          : this.selected.year;
        this.selected.month = this.params.month
          ? parseInt(this.params.month)
          : this.selected.month;
        this.selected.day = false;
      }
    },
    ...
    _buildPrevNextDate: function () {
      // prev
      var date = this._getPrevYearAndMonth(
        this.current.year,
        this.current.month,
      );
      this.prev = {
      ...
      // next
      var date = this._getNextYearAndMonth(
        this.current.year,
        this.current.month,
      );
      this.next = {
    ...
    _buildStartEndDate: function () {
      this.params.startDate = this.params.startDate
        ? this._parseDateString(this.params.startDate)
        : false;
      this.params.endDate = this.params.endDate
        ? this._parseDateString(this.params.endDate)
        : false;
    },
    ...
    _open: function (e) {
      ...
      this.animate.run(
        this.$datepicker,
        this.params.animationOpen,
        this._opened.bind(this),
      );
    },
    _opened: function () {
      this._enableEvents();
    ...
    _close: function (e) {
      ...
      this.animate.run(
        this.$datepicker,
        this.params.animationClose,
        this._closed.bind(this),
      );
    },
    _closed: function () {
      this._disableEvents();
    ...
    _enableEvents: function () {
      ...
      this.$doc.on(
        "click" + this.namespace + " touchstart" + this.namespace,
    ...
    _setPosition: function () {
      ...
      if (this.position.type === "left") {
        (left = this.position.left + "px"), (right = "auto");
      }
    ...
    _parseDateString: function (str) {
      ...
      obj.month =
        date === null
          ? this.today.month
          : this._parseMonth(format[monthIndex], date[monthIndex]);
    ...
    _convertDateToFormat: function (obj) {
      ...
      formated = formated.replace(
        "%M",
        this.lang.get("months-short")[obj.month],
      );
